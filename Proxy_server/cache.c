...
void writeToCache(char * uri, char * data, int size)
{
    ...
    /* Case 4 - evict the cache until there is enough 
     * space to add this new data
     */
    ...
    /* Case 5 - the uri is already in the cache so we update   
     * inserted time
     */
    if(newLine != NULL)
    {
    ...
    /*Case 6 - the uri is not in the cache so we create a 
     * new cacheLine and add it to the begining of the cache 
     * i.e. the front of the linked list
     */
